...
AVG_LANDMARKS = 0
num_avg_frames = 5  # num of frames to average over (Landmark)
...
class Demo:
    ...
    def run(self) -> None:
        while True:
            ...
            if DEMO:
                # pts = draw_utils_mac.display_canv(CANV_MODE=CANV_MODE, cur_pos=mid_point)  # cur_pos=cur_pos   # 수정 해야 할 부분
                # EYE_SELECT 값에 따라 custom_param 설정
                if EYE_SELECT == 0:
                    selected_point = left_point  # 왼쪽 눈 시선 좌표
                elif EYE_SELECT == 1:
                    selected_point = right_point  # 오른쪽 눈 시선 좌표
                else:
            ...
            # FIRST WE UNDISTORT THE IMAGE!   # 카메라 매트릭스와 왜곡 계수를 사용하여 cv2.unidstort() 함수로 이미지 보정
            ...
            self.visualizer.set_image(frame.copy())  # 시각화 하는 작업

            dlib_time = time.time()
            faces = self.gaze_estimator.detect_faces(undistorted)  # 얼굴을 감지함 -> dlib을 사용하여 감지된 얼굴 리스트를 반환
            if RUNTIME:
            ...
            if self.writer:  # 현재 프레임을 파익에 기록함.
                self.writer.write(self.visualizer.image)
                # self.write_eyes.write(self.visualizer.image)
            if self.config.demo.display_on_screen:  # 화면에 표시해야할 경우, 이미지를 스케일링하고, cv2.inshow()로 화면 출력
...
#########################################################
    ...
    def _draw_gaze_vector(self, face: Face) -> None:
        ...
        global mid_point  # 시선 추정에서 각 눈의 시선 벡터를 사용해 화면의 중앙 좌표인 mid point를 계산하는 부분
        if self.config.mode == GazeEstimationMethod.MPIIGaze.name:
            # -----------------------------------------------
            if GAZE_AVG_FLAG:
...
def main():
    ...
    global RUNTIME, CUST_VIDEO, vid_file, PRINT_VALS, CUSTOM_PARAM
    global EYE_SELECT
    ...
    EYE_SELECT = custom['eye_select_mode']
    ...
    if CUST_VIDEO:
        ...
        if config.mode == 'MPIIGaze':
            dump_dict(str_name, items=[demo.left_eye_cent, demo.left_eye_gaze, demo.right_eye_cent, demo.right_eye_gaze,
                                       demo.true_pos, demo.dist],
        ...
        print('CEP95 = ', CE95)
        # draw results
        draw_utils_mac.plot_pts((demo.true_pos, demo.cur_pos), str_name, MAE, save_path)  # 여기에서 시선 벡터의 시각적 표시하는 부분을 의미함. # 수정해야할 부분
